...
- **响应回显实际推理等级**：`/v1/chat/completions` 的非流式响应体与流式
  ...
  等级（模型路由配置覆盖客户端请求后的值）。
- **未兑现工具声明修复**：当客户端声明了工具、而本次轮次没有任何工具调用、
  但模型回答声称已完成文件/代码修改（如"已修正…/已更新文件…"）时，网关会
  发起一次软修复请求，要求模型输出真实的工具调用（`{"calls":[...]}` JSON /
  围栏代码块 / XML 任意编码）让客户端真正执行，避免把"声称完成"的散文当
  最终答案转发（模型常把"不用进行测试"误读为"无需真正修改"）。修复仍无
  调用时退回原文正常结束，不引入错误。默认开启，可用
  `M365_REPAIR_UNFULFILLED_TOOL_CLAIMS=0` 或后台设置
  `repairUnfulfilledToolClaims` 关闭。普通流式与推理流式（
  ChainOfThought）路径均生效。
