...
func TestIsWorkspaceToolMisjudgment(t *testing.T) {
	tests := []struct {
		...
		{name: "reported bug — files not obtained (constrained-retry trace)", text: "当前回合已先复核执行环境，但没有取得 `D:\\NET\\ai\\ythh-1` 工作区文件，因此不能安全地继续修改或提交代码，也不会伪造执行结果。\n\n根据现有证据，恢复后的第一项操作应是修正 `src/recruitment_agent/infrastructure/redis.py` 中被中断编辑破坏的函数签名。", want: true},
		{name: "reported bug — cannot safely continue modifying (standalone)", text: "不能安全地继续修改或提交代码。", want: true},
		{name: "reported bug — EN no access to workspace files", text: "I can't continue with the task as the current session doesn't provide access to the necessary workspace files for execution.", want: true},
		{name: "reported bug — EN files not obtained", text: "I did not obtain the project files, so I cannot verify the change.", want: true},
		{name: "legitimate — obtain status narration", text: "运行 `git status` 没有获取到变更，工作区是干净的。", want: false},
		{name: "legitimate — read succeeded", text: "已读取工作区文件，发现 3 处需要修改。", want: false},
		{name: "legitimate — unreadable log file", text: "我无法读取这个 800MB 的日志文件，请提供切片。", want: false},
		{name: "legitimate — safe-write refusal without workspace object", text: "为了防止数据损坏，系统无法安全地写入配置文件。", want: false},
		{name: "legitimate — EN locked file", text: "The file cannot be read because it is locked.", want: false},
		{name: "legitimate — EN session files access", text: "I don't have write access to this session's files.", want: false},
